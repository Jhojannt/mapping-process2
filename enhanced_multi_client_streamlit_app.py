...
def create_bulk_save_modal():
    ...
    if st.session_state.show_bulk_save_modal:
        ...
        if st.session_state.bulk_save_status == 'ready':
            ...
            # Liquid gradient progress bar with animation
            progress_html = f"""
            <div style="margin: 20px 0;">
                <div style="display: flex; justify-content: space-between; margin-bottom: 10px; align-items: center;">
                    <span style="font-weight: bold; font-size: 1.1em;">Progress: {st.session_state.bulk_save_progress:.1f}%</span>
                    <span style="color: #666; font-size: 0.9em;">{progress_text}</span>
                </div>
                <div style="
                    width: 100%;
                    height: 40px;
...
def create_streamlit_table_with_actions(df):
    ...
    for idx, row in df.iterrows():
        ...
        with row_cols[2]:
            # FIX: Safe similarity handling to prevent conversion errors
            try:
        ...
        with row_cols[3]:
            ...
            if catalog_id.strip() == "111111.0" or catalog_id.strip() == "111111":
...
def apply_filters(df, search_text, min_sim, max_sim, filter_column, filter_value):
    ...
    if "Similarity %" in filtered_df.columns:
        # FIX: Safe conversion to numeric, handling empty strings and non-numeric values
...
def main():
    ...
    if st.session_state.processed_data is not None:
        ...
        if len(filtered_df) > 0:
            # Progress tracking with safe similarity calculation
            ...
            try:
                # FIX: Safe average similarity calculation
                similarity_values = pd.to_numeric(df['Similarity %'], errors='coerce').dropna()
                avg_similarity = similarity_values.mean() if len(similarity_values) > 0 else 0
            except Exception:
                avg_similarity = 0
            ...
            st.markdown(
                ...
                <div class="progress-container">
                    ...
                    <p><strong>Average Similarity:</strong> {avg_similarity:.1f}%</p>
        ...
        with st.expander("📖 **Enhanced Features Guide**"):
            ...
            ### Database Features:
            - **📊 Load from DB**: Load previously processed data from database
            - **🔍 Test Connection**: Test database connectivity
            - **Automatic Reconnection**: Smart reconnection handling
            
            ### Workflow:
            1. Upload files and process data (auto-sorted by relevance)
            2. OR Load existing data from database using "📊 Load from DB" button
            3. Use bulk actions for efficient Accept/Deny marking
            4. Use bulk database save for efficient persistence (5 rows at a time)
            5. Monitor progress with real-time feedback
            6. Handle individual row edits as needed
            """)
